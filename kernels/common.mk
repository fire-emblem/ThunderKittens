...
PYBIND_INCLUDES ?= $(shell python3 -m pybind11 --includes 2>/dev/null)
ifeq ($(strip $(PYBIND_INCLUDES)),)
PYBIND_INCLUDES := $(shell python3 -c "import os, torch; print('-I' + os.path.join(os.path.dirname(torch.__file__), 'include'))")
endif
...
PYTORCH_CXX11_ABI ?= $(shell python3 -c "import torch; print(int(torch._C._GLIBCXX_USE_CXX11_ABI))")
NVCCFLAGS += -shared -fPIC # make shared object
...
NVCCFLAGS += -D_GLIBCXX_USE_CXX11_ABI=$(PYTORCH_CXX11_ABI)
NVCCFLAGS += $(PYTHON_INCLUDES) ${PYTHON_LIBDIR} $(PYBIND_INCLUDES)
