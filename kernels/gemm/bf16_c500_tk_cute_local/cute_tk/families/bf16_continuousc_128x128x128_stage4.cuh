...
struct bf16_continuousc_128x128x128_stage4 {
    ...
    template <typename T, typename Tc, typename Tscal, bool IsBetaZero,
              bool HasOneDimBias>
    static inline void launch(dim3 grid, const void *a, const void *b, void *c,
                              int m, int n, int k, int lda, int ldb, int ldc,
                              Tscal alpha_value, Tscal beta_value,
                              const void *bias = nullptr) {
        kernel::tk_local_bf16_continuousc_128x128x128_stage4<
            T, Tc, Tscal, IsBetaZero,
            HasOneDimBias><<<grid, tile::threads>>>(a, b, c, m, n, k, lda, ldb,
                                                    ldc, alpha_value,
                                                    beta_value, bias);
    }
};
