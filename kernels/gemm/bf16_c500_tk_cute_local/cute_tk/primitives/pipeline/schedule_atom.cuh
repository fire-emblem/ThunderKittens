...
// Pipeline schedule primitive - barrier scheduling for pipeline stages
struct pipeline_schedule_t {
    ...
    __device__ __forceinline__ static void wait_prologue_stage0() {
        pipeline_sync_t::template wait_gmem_async<4 * (StageCount - 1)>();
        pipeline_sync_t::barrier();
    }

    template <int StageCount>
    __device__ __forceinline__ static void wait_prologue_stage1() {
        pipeline_sync_t::template wait_gmem_async<4 * (StageCount - 2)>();
        pipeline_sync_t::barrier();
    }

    template <int StageCount>
    __device__ __forceinline__ static void wait_steady_window() {
        pipeline_sync_t::template wait_gmem_async<4 * StageCount - 10>();
        pipeline_sync_t::barrier();
    }
        ...
        switch (stage_idx) {
            ...
            case 1:
                if constexpr (StageCount >= 3) {
                    pipeline_sync_t::template wait_gmem_async<4 * (StageCount - 3)>();
                    pipeline_sync_t::barrier();
                }
                break;
            case 2:
                if constexpr (StageCount >= 4) {
                    pipeline_sync_t::template wait_gmem_async<4 * (StageCount - 4)>();
                    pipeline_sync_t::barrier();
                }
                break;
            default:
...
using schedule_atom = ::bf16_c500_tk_cute_local::primitives::pipeline_schedule_t;
}